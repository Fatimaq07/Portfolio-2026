    ...
    <AnimatePresence mode="wait">
      {skill && (
        <motion.div
          ...
          className="w-64 sm:w-full max-w-xs p-5 rounded-2xl bg-card border border-border shadow-lg will-change-[opacity]"
        >
...
export const SkillsSection = () => {
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      const factor = vw < 640 ? 0.85 : vw < 1024 ? 0.6 : 0.5;
      const size = Math.min(vw * factor, 420);
  ...
  return (
    <section ref={sectionRef} id="skills" className="relative flex flex-col justify-center overflow-hidden py-12 md:h-screen"
    >
      ...
      {/* Mobile skill cards (stacked) */}
      <div className="md:hidden flex flex-col items-center gap-4 mt-6 px-4 w-full">
        <SkillCard skill={currentLeftSkill} side="left" />
        <SkillCard skill={currentRightSkill} side="right" />
      </div>
    </section>
